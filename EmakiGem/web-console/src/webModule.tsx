...
import { PropRow, SectionHead, StandardEconomyProviderSelect, StandardEffectsEditor, StringListEditor, asList, asRecord, asStringList, coreEffectDefinition, defineCapabilities, defineEmakiPluginWebModule, defineGuiEditor, defineItemEditor, defineItemFieldRenderer, defineItemPreviewFallback, defineLocales, fieldLabel, firstItemSource, getLocale, humanizeFieldLabel, localeText, materialFromItemSource, parseActionList, payloadEffectDefinition, standardEconomyRuleFields, textValue, type AnyMap, type ConfigCreateTemplateEntry, type ConfigListItemSchemaEntry, type ConfigMetaFieldEntry, type ConfigRuleFieldEntry, type ItemFieldRendererContext, type ItemPreviewResult } from 'emaki-web-console';
import { installEmakiGemStyles } from './styles';

export const emakiGemWebModule = (() => {
  ...
  type ConfigSpec = ConfigMetaFieldEntry;
  ...
  const configFields: ConfigSpec[] = [
    ...
    ['inlay_success.by_level', '按等级成功率', '按宝石等级覆盖镶嵌成功率，key 为等级数字。', 'dynamic_map', { creatableChildren: true, createTemplates: [{ id: 'inlay-success-rate', label: copy('等级成功率', 'Level success rate'), fields: [{ path: 'value', label: '成功率', comment: '该等级镶嵌成功率百分比。', type: 'number', defaultValue: 100 }] }] }],
    ['upgrade', '升级配置', '宝石升级全局成功率、失败惩罚与等级覆盖。', 'object'],
    ['upgrade.global_success_rates', '全局升级成功率', '按目标等级配置的全局升级成功率表。', 'dynamic_map', { creatableChildren: true }],
  ...
  const configMetaFields: ConfigMetaFieldEntry[] = configFields;
  const guiEditor = defineGuiEditor({
    editorId: 'emakigem:gui',
    label: copy('宝石 GUI', 'Gem GUI'),
    fields: [
  ...
  const listItemSchemas: ConfigListItemSchemaEntry[] = [
    ['chain.pattern', [
  ...
  const createTemplates: ConfigCreateTemplateEntry[] = [
    ['socket_openers', {
      id: 'socket-opener',
  ...
  const socketItemEditorDescriptor = {
  ...
  const gemEditorFields = [
    { path: 'effects', label: '宝石效果', comment: '实际写入属性、技能和名称/Lore 动作的效果列表。', type: 'effects' },
    { path: 'name_actions', label: '名称动作链', comment: '宝石展示时对名称执行的标准动作链。', type: 'actions' },
    { path: 'lore_actions', label: 'Lore 动作链', comment: '宝石展示时对 Lore 执行的标准动作链。', type: 'actions' },
    { path: 'inlay_cost', label: '镶嵌费用', comment: '镶嵌宝石时消耗的货币与材料。', type: 'object' },
    { path: 'extract_cost', label: '拆卸费用', comment: '拆卸宝石时消耗的货币与材料。', type: 'object' },
    { path: 'extract_return', label: '拆卸返还', comment: '拆卸后宝石原样返还、销毁或降级返还。', type: 'object' }
  ];

  const socketItemEditorFields = [
    { path: 'slots', label: '插槽列表', comment: '该物品拥有的宝石插槽。', type: 'list' },
    { path: 'default_open_slots', label: '默认开放插槽', comment: '物品初始已开放的插槽索引列表。', type: 'numberList' },
    { path: 'obtain.name_actions', label: '名称动作链', comment: '获得该装备时对名称执行的动作。', type: 'actions' },
    { path: 'obtain.lore_actions', label: 'Lore 动作链', comment: '获得该装备时对 Lore 执行的动作。', type: 'actions' },
    { path: 'obtain.actions', label: '获得动作', comment: '首次识别并写入宝石层时执行的动作。', type: 'list' }
  ];

  const itemFieldRenderers = [
    defineItemFieldRenderer({ type: 'cost', renderer: context => <CostEditor label={fieldLabel(context.field.path, { moduleId: MODULE, namespace: MODULE, fallback: getLocale().startsWith('zh') ? context.field.label : humanizeFieldLabel(context.field.path) })} path={context.field.path} value={context.value ?? { currencies: [], materials: [] }} economyProviders={context.economyProviders} onChange={next => context.setField(context.field.path, next)} />, priority: 100 }),
    defineItemFieldRenderer({ type: 'extractReturn', renderer: context => <ExtractReturnEditor path={context.field.path} value={context.value} onChange={next => context.setField(context.field.path, next)} />, priority: 100 }),
    defineItemFieldRenderer({ type: 'gemUpgrade', renderer: context => <UpgradeEditor context={context} />, editorId: 'emakigem:gem', priority: 100 }),
    defineItemFieldRenderer({ type: 'gemSlots', renderer: context => <GemSlotsEditor context={context} />, editorId: 'emakigem:socket-item', priority: 100 })
  ];
  ...
  const itemPreviewFallbacks = [
    defineItemPreviewFallback({ fallback: context => localGemPreview(context.data, context.previewLevel, context.baseName, context.baseLore), editorId: 'emakigem:gem', priority: 100 }),
    defineItemPreviewFallback({ fallback: context => localSocketItemPreview(context.data, context.baseName, context.baseLore), editorId: 'emakigem:socket-item', priority: 100 })
  ];
  ...
  return defineEmakiPluginWebModule({
    module: { id: MODULE, displayName: 'Gem', summaryKey: 'emakigem.module.summary', icon: 'gem', tone: 'gem' },
    files: [
      { id: 'config', path: 'config.yml', kind: 'CONFIG', titleKey: 'emakigem.file.config.title', commentKey: 'emakigem.file.config.comment' },
      { id: 'conditions', path: 'conditions/**/*.yml', kind: 'CONFIG', titleKey: 'emakigem.file.conditions.title', commentKey: 'emakigem.file.conditions.comment' },
      { id: 'resonances', path: 'resonances/**/*.yml', kind: 'CONFIG', titleKey: 'emakigem.file.resonances.title', commentKey: 'emakigem.file.resonances.comment' },
      { id: 'gui', path: 'gui/**/*.yml', kind: 'GUI', editorId: 'emakigem:gui', titleKey: 'emakigem.file.gui.title', commentKey: 'emakigem.file.gui.comment' },
      { id: 'items', path: 'items/**/*.yml', kind: 'ITEM', editorId: 'emakigem:socket-item', titleKey: 'emakigem.file.items.title', commentKey: 'emakigem.file.items.comment' },
      { id: 'gems', path: 'gems/**/*.yml', kind: 'GEM', editorId: 'emakigem:gem', titleKey: 'emakigem.file.gems.title', commentKey: 'emakigem.file.gems.comment' }
    ],
    config: {
      metaFields: configMetaFields,
      fileSchemas: [
    ...
    itemEditors: [
      defineItemEditor({ editorId: 'emakigem:gem', descriptor: gemEditorDescriptor, fields: gemEditorFields }),
      defineItemEditor({ editorId: 'emakigem:socket-item', descriptor: socketItemEditorDescriptor, fields: socketItemEditorFields })
    ],
    itemFieldRenderers,
    itemPreviewFallbacks,
    effectTypes,
    insightDefinitions: [{ pathPrefix: 'gems/', idType: 'gem', idPath: 'id' }],
    locales: [defineLocales('zh-CN', { ...zhCN, ...zhCNDynamic }), defineLocales('en-US', enUS)],
    capabilities: defineCapabilities(['config', 'gui', 'item', 'insight', 'diagnostics']),
    diagnostics: [
      { id: 'emakigem.manifest-v2', description: 'Gem registers config, GUI, item editors, and previews through Manifest v2.', severity: 'info' },
      { id: 'emakigem.item-extension-points', description: 'Gem field renderers, preview fallbacks, and effect types are declared through Manifest v2 item extension points.', severity: 'info' }
    ]
  });
})();

export default emakiGemWebModule;
