...
| `EmakiSkills`     | `1.0.0`  | 技能系统   | 提供技能槽位、触发器、施法模式、冷却与 MythicMobs / Attribute 桥接能力                |
...
- `EmakiSkills` 通过技能解锁、触发器和施法模式承接主动技能管理，并可桥接 MythicMobs 与 EmakiAttribute。
...
├── EmakiCooking/        # 烹饪系统
├── EmakiGem/            # 装备宝石系统
├── EmakiSkills/         # 主动技能系统（private-modules profile）
└── pom.xml              # Maven 父工程
```
...
- `EmakiSkills`：技能、GUI 与触发器配置位于 `skills/`、`gui/` 与 `config.yml`，技能效果本体通过 MythicMobs 配置桥接。
