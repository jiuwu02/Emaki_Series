...
public final class OraxenBlockBridgeProvider implements CustomBlockBridge {
    ...
    private CustomBlockBridge resolveDelegate() {
        ...
        synchronized (this) {
            ...
            try {
                current = new OraxenBlockBridgeApi();
                delegate = current;
                return current;
            } catch (LinkageError exception) {
                failed = true;
                if (owner != null) {
                    owner.getLogger().warning("Failed to initialize Oraxen block API bridge: "
