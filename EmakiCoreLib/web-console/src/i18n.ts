...
const CORE_ZH_CN: LocaleMessages = {
  ...
  'core.toast.extensionLoadFailed': '{count} 个插件扩展加载失败，已保留可用页面。',
  'core.toast.refreshFailed': '刷新失败。',
  'core.toast.reloaded': '已重载当前文件。',
  ...
  'core.config.sourceLoading': '源码仍在加载，暂时无法新建字段。',
  'core.config.createdSourceField': '已在源码草稿中新增 {key}，检查后保存源码。',
  'core.config.createKicker': '结构化新建',
  'core.config.createTitle': '新建配置对象',
  'core.config.createDesc': '将在 {path} 下创建一个新对象，并把结果写入源码草稿。',
  'core.config.createKey': '新键名',
  'core.config.createTemplate': '模板',
  'core.config.createNoTemplate': '此字段还没有注册表单模板，将创建一个空对象。',
  'core.config.createEmptyTemplate': '空对象',
  'core.config.createDuplicate': '{key} 已存在，请换一个键名。',
  'core.config.create': '创建对象',
  'core.file.createTitle': '新建文件',
  'core.file.createDesc': '在当前文件夹入口下创建一个新文件。文件名可包含子目录，扩展名会按入口规则自动补全。',
  ...
  'core.file.deleteTitle': '确认删除文件',
  'core.file.deleteDesc': '此操作会删除 {path}，无法在 Web Console 内撤销。请输入完整路径确认。',
  'core.file.deleteConfirmLabel': '输入完整路径以确认删除',
  'core.file.delete': '永久删除',
