...
final class ForgeCommandRouter implements TabExecutor {
    ...
    private boolean handleDebug(CommandSender sender) {
        ...
        AppConfig current = plugin.appConfig();
        plugin.appConfigLoader().overrideCurrent(new AppConfig(
            current.language(),
        ...
        plugin.messageService().send(sender, plugin.appConfig().debug() ? "debug.enabled" : "debug.disabled");
        return true;
    }
