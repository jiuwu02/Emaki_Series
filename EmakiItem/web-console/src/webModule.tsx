...
export const emakiItemWebModule = (() => {
  ...
  const itemFieldRenderers = [
    defineItemFieldRenderer({ type: 'attributeModifiers', editorId: EDITOR_ID, priority: 100, renderer: context => <AttributeModifiersFieldEditor context={context} /> }),
    defineItemFieldRenderer({ type: 'repairMaterials', editorId: EDITOR_ID, priority: 100, renderer: context => <RepairMaterialsFieldEditor context={context} /> }),
    defineItemFieldRenderer({ type: 'setPieces', editorId: SET_EDITOR_ID, priority: 100, renderer: context => <ItemSetPiecesEditor context={context} /> }),
    defineItemFieldRenderer({ type: 'setThresholds', editorId: SET_EDITOR_ID, priority: 100, renderer: context => <ItemSetThresholdsEditor context={context} /> })
  ];

  const itemEditorDescriptor = {
    id: EDITOR_ID,
    moduleId: MODULE,
  ...
  const configMetaFields: ConfigMetaFieldEntry[] = [
    ...configFields,
    ['aliases', copy('ID 别名', 'ID Aliases'), copy('旧物品 ID 到目标 ID 的迁移映射。', 'Old item ID to target ID mappings used by rename migration.'), 'dynamic_map', { creatableChildren: true }]
  ];

  return defineEmakiPluginWebModule({
    module: { id: MODULE, displayName: 'Item', summaryKey: 'emakiitem.module.summary', icon: 'item', tone: 'item' },
    files: [
      { id: 'config', path: 'config.yml', kind: 'CONFIG', titleKey: 'emakiitem.file.config.title', commentKey: 'emakiitem.file.config.comment' },
      { id: 'items', path: 'items/**/*.yml', kind: 'ITEM', editorId: EDITOR_ID, titleKey: 'emakiitem.file.items.title', commentKey: 'emakiitem.file.items.comment' },
      { id: 'sets', path: 'sets/**/*.yml', kind: 'SET', editorId: SET_EDITOR_ID, titleKey: 'emakiitem.file.sets.title', commentKey: 'emakiitem.file.sets.comment' },
      { id: 'gui', path: 'gui/**/*.yml', kind: 'GUI', editorId: 'emakicore:gui' },
      { id: 'id-aliases', path: 'id_aliases.yml', kind: 'CONFIG', titleKey: 'emakiitem.file.id_aliases.title', commentKey: 'emakiitem.file.id_aliases.comment' }
    ],
    config: {
    ...
    itemFieldRenderers,
    effectTypes,
    fileKindLabels: [defineFileKindLabel({ kind: 'SET', label: () => getLocale().startsWith('zh') ? '套装' : 'Set' })],
    insightDefinitions: [{ pathPrefix: 'items/', idType: 'emaki_item', idPath: 'id' }],
    ...
    diagnostics: [
      { id: 'emakiitem.manifest-v2', description: 'Item registers item/set editors, surfaces, and source adapters through Manifest v2.', severity: 'info' },
      { id: 'emakiitem.item-extension-points', description: 'Item field renderers and effect types are declared through Manifest v2 item extension points.', severity: 'info' }
    ]
  });
})();

export default emakiItemWebModule;
