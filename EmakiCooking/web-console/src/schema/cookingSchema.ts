...
export const cookingRecipeStructureSchemaAst = defineSchemaAst({
  id: 'emakicooking-recipe-structures',
  moduleId: MODULE,
  pathPrefix: 'recipes/',
  fields: [
    textField({ path: 'id', label: copy('ID', 'ID'), comment: copy('烹饪配方唯一标识。', 'Unique cooking recipe id.') }),
    conditionGroupField({ path: 'condition', label: copy('完成条件', 'Completion condition'), comment: copy('配方完成时评估的 CoreLib 条件组。', 'CoreLib condition group evaluated when the recipe completes.') }),
    objectField({
      path: 'result',
      label: copy('结果', 'Result'),
      comment: copy('按分支组织的产物与动作。', 'Outputs and actions grouped by result branch.'),
      fields: [
        objectField({
          path: 'success',
          label: copy('成功结果', 'Success result'),
          comment: copy('普通完成或正确烹饪时的结果分支。', 'Result branch for normal completion.'),
          fields: [
            objectListField({
              path: 'outputs',
              label: copy('产物列表', 'Outputs'),
              comment: copy('该结果分支产出的物品列表。', 'Items emitted by this result branch.'),
              itemFields: [
                stringListField({ path: 'item_sources', label: copy('产物来源', 'Output sources'), comment: copy('产物 ItemSource 列表。', 'Output ItemSource list.') }),
                numberField({ path: 'amount', label: copy('数量', 'Amount'), comment: copy('生成的产物数量。', 'Generated item amount.'), defaultValue: 1 }),
                numberField({ path: 'chance', label: copy('概率', 'Chance'), comment: copy('产出概率；留空时按必定产出处理。', 'Output chance; blank means guaranteed.'), defaultValue: 100 })
              ]
            }),
            actionStringListField({ path: 'actions', label: copy('结果动作', 'Result actions'), comment: copy('该结果分支触发后执行的 CoreLib Action 字符串列表。', 'CoreLib action command strings executed by this branch.') })
          ]
        })
      ]
    })
...
export const cookingRecipeStructureManifestSchema = defineConfigSchema(cookingRecipeStructureSchemaAst);
export const cookingPluginConfig: WebManifestPluginConfig = {
    metaFields: fields,
    fileSchemas: [
