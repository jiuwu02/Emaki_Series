import { getLocale, localeText, registerModuleLocale, registerPluginConfig, registerPluginGuiEditor } from 'emaki-web-console';
...
export function registerEmakiCookingWebConsole(): void {
  ...
  const recipeCommonFields: FieldSpec[] = [
    ...
    ['condition.required_count', '需要满足数量', 'at_least / exactly 条件逻辑下需要满足的条件数量。', 'number'],
  ...
  const wokRecipeFields: FieldSpec[] = [
    ...
    ['result.overcooked', '过熟结果', '炒锅翻炒过度时的结果分支。', 'object', { itemFields: outcomeFields }],
    ['result.overcooked.outputs', '过熟产物', '过熟结果分支产出的物品列表。', 'objectList', { itemFields: outputFields }],
  ...
  const fermentationBarrelRecipeFields: FieldSpec[] = [
    ...
    ['result.over', '过发酵结果', '发酵超时后的结果分支。', 'object', { itemFields: outcomeFields }],
  ...
  const fieldComments: Record<string, [string, string, string] | [string, string, string, { options?: string[]; optionLabelPrefix?: string }]> = {
    ...
    type: ['条件逻辑', '条件表达式组合方式。', 'enum', { options: ['all_of', 'any_of', 'none_of', 'at_least', 'exactly'], optionLabelPrefix: 'conditionType' }],
    required_count: ['需要满足数量', 'at_least / exactly 条件逻辑下需要满足的条件数量。', 'number'],
    entries: ['条件表达式', 'CoreLib 条件表达式字符串列表。', 'stringList'],
    on_pass: ['条件成立分支', '条件成立时执行的配置。', 'object'],
    on_fail: ['条件失败分支', '条件不成立时执行的配置。', 'object'],
    block_output: ['失败阻止产出', '条件不成立时是否阻止产物输出。', 'boolean'],
    id: ['ID', '配方唯一标识。', 'text'],
  ...
  registerPluginConfig({
    ...
    listItemSchemas: [
      ...
      ['result.invalid.outputs', outputFields],
      ['result.perfect.outputs', outputFields],
      ['result.overbaked.outputs', outputFields],
