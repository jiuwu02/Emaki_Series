...
const fields = [
  ...
  ['broadcast', '广播', '强化成功时的本地广播与全服广播触发星级配置。', 'object'],
  ['broadcast.local_stars', '本地广播星级', '强化成功达到这些星级时向附近玩家广播。', 'list'],
  ['broadcast.global_stars', '全服广播星级', '强化成功达到这些星级时向全服广播。', 'list'],
  ['success_rates', '成功率', '配方未单独覆盖时使用的全局强化成功率表；key 为目标星级，value 为百分比。', 'object']
] as const;
...
registerModuleLocale(MODULE, 'zh-CN', {
  ...
  'emakistrengthen.field.confirm': '确认强化'
});
...
registerPluginGuiEditor({
  ...
  fields: [
    ...
    ['success_preview', '成功率预览', '显示当前强化成功率与目标星级。', 'text'],
