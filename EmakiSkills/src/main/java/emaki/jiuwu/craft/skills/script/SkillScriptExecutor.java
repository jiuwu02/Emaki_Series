...
public final class SkillScriptExecutor {
    ...
    public CompletableFuture<ActionResult> executePhase(SkillScriptContext context,
        ...
        if (!conditions.isEmpty() && !ConditionEvaluator.evaluate(conditions, "all_of", null,
            ...
            return CompletableFuture.completedFuture(ActionResult.skipped("Script phase condition did not pass."));
        }
        List<String> lines = script.lines(phase);
        if (lines.isEmpty()) {
            return CompletableFuture.completedFuture(ActionResult.ok());
        }
        ...
        CompletableFuture<ActionResult> future = CompletableFuture.completedFuture(ActionResult.ok());
    ...
    public CompletableFuture<ActionResult> executeLine(SkillScriptContext context, int lineNumber, String line) {
        ParsedActionLine parsed;
        try {
            parsed = lineParser.parse(lineNumber, resolveText(context, line));
        } catch (ActionSyntaxException exception) {
            return CompletableFuture.completedFuture(ActionResult.failure(ActionErrorType.SYNTAX_ERROR, exception.getMessage()));
        }
        if (parsed == null) {
            return CompletableFuture.completedFuture(ActionResult.ok());
        }
        ...
        if (Texts.isNotBlank(condition)) {
            ...
            if (passes == null) {
                return CompletableFuture.completedFuture(ActionResult.failure(ActionErrorType.INVALID_ARGUMENT,
        ...
        if (action != null) {
            ActionResult validation = action.validate(arguments);
    ...
    private ActionResult afterLine(SkillScriptContext context, ActionResult result) {
