import { defineCapabilities, defineEmakiPluginWebModule, defineGuiEditor, defineLocales, localeText, standardCurrencyCostFields, standardMaterialCostFields } from 'emaki-web-console';
...
const parameterFields = [
  ...
  { path: 'formula', label: '公式', comment: '资源消耗或效果计算公式。', type: 'text', defaultValue: '' },
  ...
  { path: 'chars', label: '候选字符', comment: 'random_char/weighted_random_char 的候选字符。', type: 'text', defaultValue: '' },
  { path: 'weights', label: '权重列表', comment: 'weighted_random_char 的权重列表。', type: 'numberList', defaultValue: [] },
  { path: 'count', label: '随机次数', comment: '随机字符抽取次数。', type: 'number', defaultValue: 1 },
...
const fields: FieldSpec[] = [
  ...
  ['cast_mode.entry_key', '切换按键', '进入或退出施法模式的按键标识。', 'text'],
...
export const emakiSkillsWebModule = defineEmakiPluginWebModule({
  module: { id: MODULE, displayName: 'Skills', summaryKey: 'emakiskills.module.summary', icon: 'skills', tone: 'skills' },
  files: [
    { id: 'config', path: 'config.yml', kind: 'CONFIG', titleKey: 'emakiskills.file.config.title', commentKey: 'emakiskills.file.config.comment' },
    { id: 'skills', path: 'skills/**/*.yml', kind: 'CONFIG', titleKey: 'emakiskills.file.skills.title', commentKey: 'emakiskills.file.skills.comment' },
    { id: 'resources', path: 'resources/**/*.yml', kind: 'CONFIG', titleKey: 'emakiskills.file.resources.title', commentKey: 'emakiskills.file.resources.comment' },
    { id: 'gui', path: 'gui/**/*.yml', kind: 'GUI', editorId: 'emakiskills:gui', titleKey: 'emakiskills.file.gui.title', commentKey: 'emakiskills.file.gui.comment' }
  ],
  insightDefinitions: [{ pathPrefix: 'skills/', idType: 'skill', idPath: 'id' }],
  config: {
    metaFields: fields,
    fileSchemas: [
  ...
  diagnostics: [
    { id: 'emakiskills.manifest-v2', description: 'Skills registers config and GUI editors through Manifest v2.', severity: 'info' },
    { id: 'emakiskills.script-schema', description: 'Skills script phases, resources, upgrade costs, and parameters are represented in Manifest v2 config metadata.', severity: 'info' }
  ]
});

export default emakiSkillsWebModule;
