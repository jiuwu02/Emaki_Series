...
export function registerEmakiAttributeWebConsole(): void {
  ...
  const commonFields: Record<string, [string, string, string]> = {
    ...
    type: ['条件逻辑', '多条件组合逻辑。', 'enum'],
    invalid_as_failure: ['解析失败视为失败', '条件表达式解析失败时是否视为不通过。', 'boolean'],
    required_count: ['需要满足数量', 'at_least / exactly 条件逻辑下需要满足的最少条件数量。', 'number'],
    schema_version: ['结构版本', '条件规则结构版本。', 'number'],
    entries: ['条件列表', '具体条件项列表，每项包含 type、key/pattern 和 condition 表达式。', 'list'],
  ...
  const conditionFields: ConfigSpec[] = [
    ['id', 'ID', '条件定义唯一标识。', 'text'],
    ['source_id', '来源 ID', '条件规则来源标识，用于日志和调试追踪。', 'text'],
    ['schema_version', '结构版本', '条件规则结构版本。新条件建议使用 2。', 'number'],
    ['condition', '条件块', 'PDC 条件组合逻辑与条目列表。', 'object'],
    ['condition.type', '条件逻辑', '多条件组合逻辑。', 'enum', { options: ['all_of', 'any_of', 'none_of', 'at_least', 'exactly'], optionLabelPrefix: 'conditionType' }],
    ['condition.required_count', '需要满足数量', 'at_least / exactly 条件逻辑下需要满足的最少条件数量。', 'number'],
    ['condition.invalid_as_failure', '解析失败视为失败', '条件表达式解析失败时是否视为不通过。', 'boolean'],
    ['condition.entries', '条件列表', '具体条件项列表，每项包含 type、key/pattern 和 condition 表达式。', 'objectList']
  ];
  ...
  registerModuleLocale(MODULE, 'zh-CN', {
    ...
    'emakiattribute.file.attributes.title': '属性',
    'emakiattribute.file.attributes.comment': '属性文件目录，每个文件配置一个属性的 ID、类型、范围和词条格式。',
    'emakiattribute.file.damage_types.title': '伤害类型',
    'emakiattribute.file.damage_types.comment': '伤害类型文件目录，每个文件配置一种伤害的结算阶段和恢复规则。',
    ...
    'emakiattribute.diagnostics.toolbar.title': '玩家属性追踪 / 伤害调试器',
    'emakiattribute.diagnostics.toolbar.subtitle': '查询在线玩家的属性来源与最近伤害 Trace',
    'emakiattribute.diagnostics.outline.title': '诊断',
    'emakiattribute.diagnostics.outline.empty': '输入玩家名后查询。',
    'emakiattribute.diagnostics.error.playerRequired': '请输入在线玩家名。',
    'emakiattribute.diagnostics.error.playerNotFound': '未找到在线玩家：{player}。请填写执行 /ea debug <player> on 时使用的在线玩家名，不要填写怪物或实体名。',
    'emakiattribute.diagnostics.error.playerNotFoundUnknown': '未找到在线玩家。请填写执行 /ea debug <player> on 时使用的在线玩家名，不要填写怪物或实体名。',
    'emakiattribute.diagnostics.error.queryFailed': '查询失败',
    'emakiattribute.diagnostics.filters.title': '查询条件',
    'emakiattribute.diagnostics.filters.player': '玩家名',
    'emakiattribute.diagnostics.filters.attributeId': '属性 ID（可选）',
    'emakiattribute.diagnostics.action.loadSources': '加载属性来源',
    'emakiattribute.diagnostics.action.refreshTraces': '刷新伤害 Trace',
    'emakiattribute.diagnostics.action.clearTraces': '清空 Trace',
    'emakiattribute.diagnostics.traceHint': '伤害 Trace 需要先在游戏内执行 /ea debug <player> on 开启；这里填写开启追踪的在线玩家名（攻击者或被攻击玩家），不是怪物/实体名。',
    'emakiattribute.diagnostics.finalAttributes.title': '最终属性',
    'emakiattribute.diagnostics.finalAttributes.empty': '暂无属性快照。',
    'emakiattribute.diagnostics.contributions.title': '来源贡献',
    'emakiattribute.diagnostics.contributions.empty': '暂无来源记录。',
    'emakiattribute.diagnostics.column.attribute': '属性',
    'emakiattribute.diagnostics.column.value': '值',
    'emakiattribute.diagnostics.column.source': '来源',
    'emakiattribute.diagnostics.column.slot': '槽位',
    'emakiattribute.diagnostics.column.status': '状态',
    'emakiattribute.diagnostics.status.inactive': '未生效',
    'emakiattribute.diagnostics.status.active': '生效',
    'emakiattribute.diagnostics.traces.title': '最近伤害 Trace',
    'emakiattribute.diagnostics.traces.empty': '暂无 Trace。',
    'emakiattribute.diagnostics.stage': '阶段',
    'emakiattribute.diagnostics.json.title': 'JSON 导出',
    'emakiattribute.diagnostics.json.empty': '查询后显示最近一次结果。',
    ...
    'emakiattribute.filePath.conditions_emakiitem.comment': 'EmakiItem 条件，按自定义物品身份限制属性读取。',
    'emakiattribute.filePath.conditions_forge.comment': '锻造条件，按锻造来源或结果限制属性读取。',
    'emakiattribute.filePath.conditions_strengthen.comment': '强化条件，按强化状态限制属性读取。',
    'emakiattribute.file.plugin.title': '插件描述',
    'emakiattribute.file.plugin.comment': 'plugin.yml 元数据、命令、权限和依赖声明。',
    'emakiattribute.file.web-console.title': 'WebUIEdit 注册',
    'emakiattribute.file.web-console.comment': '此插件暴露给 WebUIEdit 的文件分组、编辑器类型和前端扩展入口。',
    ...
    'emakiattribute.option.conditionType.all_of': '全部满足',
    'emakiattribute.option.conditionType.any_of': '任一满足'
  });
  ...
  registerModuleLocale(MODULE, 'en-US', {
    ...
    'emakiattribute.file.attributes.title': 'Attributes',
    'emakiattribute.file.attributes.comment': 'Directory of attribute files. Each file configures an attribute ID, type, range, and lore format.',
    'emakiattribute.file.damage_types.title': 'Damage Types',
    'emakiattribute.file.damage_types.comment': 'Directory of damage type files. Each file configures a damage settlement stage and recovery rules.',
    ...
    'emakiattribute.diagnostics.toolbar.title': 'Attribute sources / Damage trace',
    'emakiattribute.diagnostics.toolbar.subtitle': 'Query attribute sources and recent damage traces for an online player',
    'emakiattribute.diagnostics.outline.title': 'Diagnostics',
    'emakiattribute.diagnostics.outline.empty': 'Enter an online player name to query.',
    'emakiattribute.diagnostics.error.playerRequired': 'Enter an online player name.',
    'emakiattribute.diagnostics.error.playerNotFound': 'Online player not found: {player}. Enter the online player name used with /ea debug <player> on, not a mob or entity name.',
    'emakiattribute.diagnostics.error.playerNotFoundUnknown': 'Online player not found. Enter the online player name used with /ea debug <player> on, not a mob or entity name.',
    'emakiattribute.diagnostics.error.queryFailed': 'Query failed',
    'emakiattribute.diagnostics.filters.title': 'Query filters',
    'emakiattribute.diagnostics.filters.player': 'Player name',
    'emakiattribute.diagnostics.filters.attributeId': 'Attribute ID (optional)',
    'emakiattribute.diagnostics.action.loadSources': 'Load attribute sources',
    'emakiattribute.diagnostics.action.refreshTraces': 'Refresh damage traces',
    'emakiattribute.diagnostics.action.clearTraces': 'Clear traces',
    'emakiattribute.diagnostics.traceHint': 'Damage traces must first be enabled in-game with /ea debug <player> on. Enter the tracked online player name here, attacker or target, not a mob or entity name.',
    'emakiattribute.diagnostics.finalAttributes.title': 'Final attributes',
    'emakiattribute.diagnostics.finalAttributes.empty': 'No attribute snapshot yet.',
    'emakiattribute.diagnostics.contributions.title': 'Source contributions',
    'emakiattribute.diagnostics.contributions.empty': 'No source records yet.',
    'emakiattribute.diagnostics.column.attribute': 'Attribute',
    'emakiattribute.diagnostics.column.value': 'Value',
    'emakiattribute.diagnostics.column.source': 'Source',
    'emakiattribute.diagnostics.column.slot': 'Slot',
    'emakiattribute.diagnostics.column.status': 'Status',
    'emakiattribute.diagnostics.status.inactive': 'Inactive',
    'emakiattribute.diagnostics.status.active': 'Active',
    'emakiattribute.diagnostics.traces.title': 'Recent damage traces',
    'emakiattribute.diagnostics.traces.empty': 'No traces yet.',
    'emakiattribute.diagnostics.stage': 'Stage',
    'emakiattribute.diagnostics.json.title': 'JSON export',
    'emakiattribute.diagnostics.json.empty': 'The latest result appears after a query.',
    ...
    'emakiattribute.filePath.conditions_emakiitem.comment': 'EmakiItem condition for limiting attribute reads by custom item identity.',
    'emakiattribute.filePath.conditions_forge.comment': 'Forge condition for limiting attribute reads by forge source or result.',
    'emakiattribute.filePath.conditions_strengthen.comment': 'Strengthen condition for limiting attribute reads by strengthen state.',
    'emakiattribute.file.plugin.title': 'Plugin Description',
    'emakiattribute.file.plugin.comment': 'plugin.yml metadata, commands, permissions, and dependency declarations.',
    'emakiattribute.file.web-console.title': 'WebUIEdit Registration',
    'emakiattribute.file.web-console.comment': 'File groups, editor kinds, and frontend extension entries exposed to WebUIEdit by this plugin.',

    'emakiattribute.field.hard_lock_damage': 'Hard-lock Damage',
    'emakiattribute.field.default_damage_type': 'Default Damage Type',
    'emakiattribute.field.vanilla_event_damage': 'Vanilla Event Damage',
    'emakiattribute.field.vanilla_event_damage.enabled': 'Enable Vanilla Damage Handling',
    'emakiattribute.field.vanilla_event_damage.damage_type': 'Damage Type',
  ...
  registerPluginConfig({
    ...
    rules: [
      ...
      [{ path: 'condition.type' }, { label: copy('条件逻辑', 'Condition logic'), comment: '多条件组合逻辑。', type: 'enum', options: ['all_of', 'any_of', 'none_of', 'at_least', 'exactly'], optionLabelPrefix: 'conditionType' }],
      ...
      [{ path: 'condition.required_count' }, { label: copy('需要满足数量', 'Required count'), comment: 'at_least / exactly 条件逻辑下需要满足的最少条件数量。', type: 'number' }]
    ],
    createTemplates: [
