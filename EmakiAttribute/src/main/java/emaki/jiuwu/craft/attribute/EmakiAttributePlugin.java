...
""";
    ...
    private void registerWebConsole() {
        WebConsoleRegistry.registerModule(getName(), "Attribute 属性", "属性、资源、伤害接管与曲线", "attribute", WEB_ICON);
        ...
        WebConsoleRegistry.registerNodeKeyComment(getName(), "display_name", "显示名称", "资源或属性在界面中显示的名称。", "text");
        ...
        WebConsoleRegistry.registerConfigFile(getName(), "属性权重配置", "attribute_balance.yml", "属性语义分组、角色定位与评分权重配置。");
        WebConsoleRegistry.registerNodeComment(getName(), "attributes", "属性语义", "每个属性的分组、角色和描述定义。", "object");
        WebConsoleRegistry.registerNodeComment(getName(), "scores", "评分权重", "各属性在装备评分中的权重系数。", "object");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "group", "分组", "属性所属的功能分组（如 physical、spell、utility）。", "text");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "role", "角色", "属性在战斗中的角色定位（如 offense、defense、sustain）。", "text");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "summary", "描述", "属性的简短功能描述，用于前端和文档展示。", "text");
        ...
        WebConsoleRegistry.registerNodeKeyComment(getName(), "max_value", "最大值", "属性允许的最大值，超过此值会被钳制。", "number");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "allow_negative", "允许负值", "是否允许属性值为负数。", "boolean");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "priority", "优先级", "词条解析优先级，数值越大越先匹配。", "number");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "lore_format_id", "词条格式", "关联的词条格式 ID，决定属性在物品 Lore 中的显示方式。", "text");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "lore_patterns", "词条正则", "用于从物品 Lore 中识别该属性数值的正则表达式列表。", "list");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "attribute_power", "属性战力", "该属性每 1 点对应的战力评分系数。", "number");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "description", "说明", "属性的详细说明文本，用于文档和调试输出。", "text");

        // ─── damage_types/**/*.yml ───
        WebConsoleRegistry.registerConfigFile(getName(), "伤害类型定义", "damage_types/**/*.yml", "伤害类型定义文件目录，每个文件定义一种伤害的结算阶段和恢复规则。");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "aliases", "别名", "伤害类型的别名列表，可通过别名引用该伤害类型。", "list");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "allowed_events", "允许事件", "允许触发该伤害类型的 Bukkit DamageCause 列表。", "list");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "hard_lock", "硬锁定", "是否强制接管该伤害类型对应的原版伤害事件。", "boolean");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "stages", "结算阶段", "伤害结算的有序阶段列表，按顺序依次执行。", "list");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "recovery", "恢复规则", "伤害造成后的生命恢复（吸血）规则配置。", "object");
        ...
        // ─── lore_formats/**/*.yml ───
        WebConsoleRegistry.registerConfigFile(getName(), "词条格式定义", "lore_formats/**/*.yml", "词条格式定义文件目录，每个文件定义一种属性在物品 Lore 中的显示模板。");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "format", "格式模板", "词条显示格式，支持 {name}、{value}、{sign} 等占位符。", "text");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "precision", "精度", "数值显示的小数位数。", "number");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "read_priority", "读取优先级", "从 Lore 解析属性时的匹配优先级，数值越大越先尝试。", "number");
        WebConsoleRegistry.registerNodeKeyComment(getName(), "read_patterns", "读取正则", "用于从 Lore 文本中提取数值的正则表达式列表。", "list");
        ...
        WebConsoleRegistry.registerNodeSuffixComment(getName(), "conditions.require_match", "必须匹配", "是否要求正则必须命中才视为通过。", "boolean");
