...
public final class ForgeConfigPrecheckContributor implements ConfigPrecheckContributor {
    ...
    public ConfigPrecheckResult check(CoreLibConfig config, ConfigPrecheckContext context) {
        ...
        if (issues.isEmpty()) {
            issues.add(ConfigPrecheckIssue.of(module(), "config.yml", ConfigPrecheckSeverity.INFO, "Forge config precheck passed."));
        }
