import { coreEffectDefinition, defineCapabilities, defineEmakiPluginWebModule, defineGuiEditor, defineLocales, localeText, payloadEffectDefinition, standardMaterialCostFields } from 'emaki-web-console';
...
const localeMessages: Record<string, string> = Object.fromEntries([
  ...
  ['emakiforge.file.gui.comment', '锻造、配方书与配方编辑器的 GUI 模板。'],
  ['emakiforge.file.recipes.title', '配方文件'],
  ['emakiforge.file.recipes.comment', '锻造配方文件目录。'],
  ['emakiforge.filePath.recipes_example_recipe.title', '示例配方'],
  ['emakiforge.filePath.recipes_example_recipe.comment', '锻造配方示例，展示材料效果、品质、费用、动作和失败结果。'],
  ['emakiforge.filePath.gui_editor_gui.comment', '配方编辑器 GUI 模板，控制游戏内编辑界面的槽位和按钮。'],
  ['emakiforge.filePath.gui_forge_gui.comment', '锻造 GUI 模板，控制玩家执行锻造时看到的界面。'],
  ['emakiforge.filePath.gui_recipe_book.comment', '配方书 GUI 模板，控制配方浏览和分页入口。'],
  ['emakiforge.file.plugin.title', '插件描述'],
  ['emakiforge.file.plugin.comment', 'plugin.yml 元数据、命令、权限和依赖声明。'],
  ['emakiforge.file.web-console.title', 'WebUIEdit 注册'],
...
const effectTypes = [
  ...
  payloadEffectDefinition('ea_attribute', 'EA 属性', [
    { key: 'ea_attributes', type: 'map', label: 'EA 属性', defaultValue: {} }
  ]),
  payloadEffectDefinition('es_skill', 'ES 技能', [{ key: 'es_skills', type: 'stringList', label: 'ES 技能', defaultValue: [] }]),
  coreEffectDefinition('name_action'),
  coreEffectDefinition('lore_action'),
  payloadEffectDefinition('quality_modify', '品质修改', [
    { key: 'mode', type: 'enum', label: '品质修改模式', options: ['force', 'minimum'], defaultValue: 'minimum' },
    { key: 'tier', type: 'text', label: '目标品质', defaultValue: '' }
  ]),
  payloadEffectDefinition('capacity_bonus', '容量加成', [{ key: 'value', type: 'text', label: '容量加成', defaultValue: '1' }])
];

export const emakiForgeWebModule = defineEmakiPluginWebModule({
  module: { id: MODULE, displayName: 'Forge', summaryKey: 'emakiforge.module.summary', icon: 'forge', tone: 'forge' },
  files: [
    { id: 'config', path: 'config.yml', kind: 'CONFIG', titleKey: 'emakiforge.file.config.title', commentKey: 'emakiforge.file.config.comment' },
    { id: 'recipes', path: 'recipes/**/*.yml', kind: 'CONFIG', titleKey: 'emakiforge.file.recipes.title', commentKey: 'emakiforge.file.recipes.comment' },
    { id: 'gui', path: 'gui/**/*.yml', kind: 'GUI', editorId: 'emakiforge:gui', titleKey: 'emakiforge.file.gui.title', commentKey: 'emakiforge.file.gui.comment' }
  ],
  effectTypes,
  insightDefinitions: [{ pathPrefix: 'recipes/', idType: 'forge_recipe', idPath: 'id' }],
  config: {
    metaFields: fields,
    fileSchemas: [
    ...
    ruleFields,
    rules: [
      [qualityTierDisplayMatcher, { itemFields: qualityTierDisplayFields } as AnyMap]
    ],
    createTemplates: [
  ...
  locales: [
    ...
    defineLocales('en-US', {
      ...
      'emakiforge.file.recipes.comment': 'Directory for forge recipe files.',
      'emakiforge.filePath.recipes_example_recipe.title': 'Sample Recipe',
      'emakiforge.filePath.recipes_example_recipe.comment': 'Forge recipe example covering material effects, quality, costs, actions, and failure outcomes.',
      'emakiforge.filePath.gui_editor_gui.comment': 'Recipe editor GUI template controlling in-game editor slots and buttons.',
      'emakiforge.filePath.gui_forge_gui.comment': 'Forge GUI template shown to players when they run a forge attempt.',
      'emakiforge.filePath.gui_recipe_book.comment': 'Recipe book GUI template controlling browsing and pagination.',
      'emakiforge.file.plugin.title': 'Plugin Description',
      'emakiforge.file.plugin.comment': 'plugin.yml metadata, commands, permissions, and dependency declarations.',
      'emakiforge.file.web-console.title': 'WebUIEdit Registration',
      'emakiforge.file.web-console.comment': 'File groups, editor kinds, and frontend extension entries exposed to WebUIEdit by this plugin.',
  ...
  capabilities: defineCapabilities(['config', 'gui', 'insight', 'diagnostics']),
  diagnostics: [
    { id: 'emakiforge.manifest-v2', description: 'Forge registers config, GUI, effect types, and insight definitions through Manifest v2.', severity: 'info' },
    { id: 'emakiforge.material-effect-schema', description: 'Forge material effects, quality tiers, blueprints, and failure outcomes are represented in Manifest v2 config metadata.', severity: 'info' }
  ]
});

export default emakiForgeWebModule;
